...
### [2026-05-22] — Debug Try/Except Pattern for Exposing Raw Uvicorn Crashes as JSON 500
**What happened:** DOCX export was returning HTTP 500 as `text/plain` (raw uvicorn crash), not a FastAPI JSON error. This meant the exception was escaping the FastAPI error handler entirely. To surface the traceback, wrapped the `render_docx()` call in a try/except that re-raises as `HTTPException(500, detail=f"render_docx failed: {type(e).__name__}: {e}\n{traceback.format_exc()}")`. This turned the opaque text/plain crash into a readable JSON 500 with full Python traceback. After identifying and fixing the root cause, the debug wrapper should be removed.
**Pattern to note:** When an async FastAPI endpoint returns text/plain 500 (not JSON), the exception is likely happening in a sync function called from async context, or in middleware. The debug try/except wrapper is the fastest way to surface the actual error. Remove it once the root cause is fixed.
